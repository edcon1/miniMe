...
bool EntityEditorApp::startup() {
	...
	m_entityCount = CreateFileMapping(
	 ...
	 unsigned int* data = (unsigned int*)MapViewOfFile(m_entityCount, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(unsigned int));

	 *data = ENTITY_COUNT;
	 UnmapViewOfFile(data);

	 m_entityHandle = CreateFileMapping(
		 INVALID_HANDLE_VALUE,
		 nullptr,
		 PAGE_READWRITE,
		 0, sizeof(Entity) * ENTITY_COUNT,
		 L"SharedEntityMemory");
...
{
	CloseHandle(m_entityCount);
	CloseHandle(m_entityHandle);
	delete m_font;
	delete m_2dRenderer;
}
...
void EntityEditorApp::update(float deltaTime) {
	...
	Entity* data = (Entity*)MapViewOfFile(m_entityHandle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(Entity) * ENTITY_COUNT);
